            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `px-4 py-2 text-md font-bold border-b border-transparent transition duration-300 ${
                  (isActive || location.hash === route.to.split('#')[1]) ? 'text-primer' : 'text-sekunder hover:text-primer hover:border-b-2 hover:border-primer hover:scale-105'
                }`
              }
            >
              {route.name}
              {(location.pathname + location.hash) === route.to && (
                <motion.div
